    ...
    <article className="relative flex min-h-screen w-full flex-col overflow-auto bg-background">
      <div className="relative z-10 flex grow flex-col">
        <div className="mx-auto w-full max-w-[1360px] px-4 pb-20 pt-28 sm:px-8 md:pt-32">
          <div className="mb-10 grid gap-8 md:mb-14 md:grid-cols-[minmax(0,200px)_1fr] md:gap-12 xl:grid-cols-[minmax(0,220px)_1fr]">
            <aside className="order-2 flex flex-row items-center gap-4 md:order-1 md:flex-col md:items-start md:gap-4 text-sm text-muted-foreground md:pt-1">
              <time dateTime={post.date}>
                {dateFormatter.format(new Date(post.date))}
              </time>
              {post.authors.map((author) => (
                <div className="flex flex-row items-center gap-2 md:gap-2.5" key={author.name}>
                  {author.avatar ? (
                    <Image
                      src={author.avatar}
                      alt=""
                  ...
                  <span className="hidden md:block leading-tight text-sm">{author.name}</span>
                </div>
              ))}
            </aside>
